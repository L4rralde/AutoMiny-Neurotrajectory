...
class NeuroEvolutionary:
    def __init__(self,  
        ...
        print("Hello from NeuroEvolutionary.init()")
        self.kinematic_ga = KinematicGA(shape, num_actions)
        self.car = Car(5, 27, max_vel, max_steering=max_steer)
    ...
    def run_ga(self, nn_model, dists):
        ...
        if self.predicted_action != Actions.reverse.value:
            apply_action(self.predicted_action, self.car, dt)
        self.car.update(dt)
        ...
        self.predicted_action = np.argmax(prediction[0])
...
class KinematicGA(object):
    def __init__(self, shape, num_actions):
        ...
        self.file_path = rospack.get_path('fub_navigation') + '/scripts/'
    ...
    def load_model(self, model_name):
        json_file = open(self.file_path +'/used_models/ga/' + model_name + '.json', 'r')
        loaded_model_json = json_file.read()
        json_file.close()
        self.model = model_from_json(loaded_model_json)
        # load weights into new model
        self.model.load_weights(self.file_path +"/used_models/ga/" + model_name + ".h5")
